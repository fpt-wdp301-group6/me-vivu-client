...
const Detail: FC<DetailProps> = ({ data }) => {
    ...
    const backdropStyles = 'h-full top-0 left-0 right-0 bottom-0 -z-1 bg-gradient-to-t from-dark to-black/20';

    return (
        <>
            <div
                className="h-[50vh] bg-cover bg-top bg-no-repeat"
                style={{
                    backgroundImage: `url(${constants.tmdbUrl}/original/${data.backdrop_path})`,
                }}
            >
                <div className={clsx(backdropStyles)}></div>
            </div>
            <Container className="-mt-[200px]">
                <div className="grid grid-cols-5 max-w-4xl mx-auto gap-6">
                    ...
                    <div className="col-span-full md:col-span-3">
                        <Stack gap={2}>
                            <h1 className="font-bold text-4xl pt-4">{data.title}</h1>
